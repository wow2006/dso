...
inline int computeHistQuantil(int *hist, float below) {
  constexpr int maxValue = 90;
  int th = static_cast<int>(hist[0] * below + 0.5f);
...
PixelSelector::PixelSelector(int w, int h)
    : currentPotential(3), allowFast(false), randomPattern(w * h),
      gradHist(100 * (1 + w / 32) * (1 + h / 32)),
      ths((w / 32) * (h / 32) + 100), thsSmoothed((w / 32) * (h / 32) + 100) {
  // want to be deterministic.
  constexpr int randomSeed = 3141592;
...
PixelSelector::~PixelSelector() = default;
...
int PixelSelector::makeMaps(const FrameHessian *const fh, float *map_out,
...

  {
    ...
    // round down.
    idealPotential = static_cast<int>(sqrtf(K / numWant) - 1);
...
Eigen::Vector3i PixelSelector::select(const FrameHessian *const frameHessian,
  ...
  const float dw1 = setting_gradDownweightPerLevel;
    ...
    for (int x4 = 0; x4 < w; x4 += (4 * pot)) {
      ...
      for (int y3 = 0; y3 < my3; y3 += (2 * pot)) {
        for (int x3 = 0; x3 < mx3; x3 += (2 * pot)) {
          ...
          const Vec2f dir3 = directions[randomPattern[n2] & 0xF];
